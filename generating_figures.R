# Making figures for 500-patch simulations
# first, read in last gens file
library(dplyr)
library(tidyverse)
library(ggplot2)

# First, read in and manipulate the data. Because I couldn't write a loop when I did this, buckle your seatbelts...I'm taking the long route:')
...
# Find the furthest patch that each rep spread
group_by(empty53l, X)%>%
  dplyr::summarize(max_dist=max(dist_patches))->empty53max

# Adding in some treatment categories and genotype categories that will be useful for manipulating data down the road
empty53max$rep<-empty53max$X
empty53max$type<-"m empty, lambda empty"
empty53max$RIL<-"53"
# dividing by the number of generations to get invasion speed
...
### Okay yay, done!
### Now, combine all sheets
master<-rbind(sheet53, sheet58, sheet144, sheet187)
master$speedcm<-master$speed*8

# Read in a package that will help us reorder the levels of our treatment variables
library(plyr)
...
# Plotting points for new manuscript figure November 2023
ggplot(data=master, aes(x=RIL, y=speed, colour=type))+
  geom_point()+
  geom_jitter()+
  xlab("parameters affected by competition")+
...
## calculating percent reductions in m
# First, read in the dispersal spreadsheet that we generated using the fitting_kernels script
parms_raw<-read.csv("simulationsformanuscript_finaldecision_july2023/individual_wholepot_parameters_compiled_sept2023.csv")
...
ggplot(parms_raw, aes(genotype, meandisp, fill=treatment))+
  geom_boxplot(stat="boxplot")+
  xlab("treatment")+
